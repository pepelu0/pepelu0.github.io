...
4. 增加读写效率
   ...
   下面的图表展示了单个命令，5个和10个命令的pipelines情况下测量的每分秒请求数。
...
5. 结论
   ...
   多线程天然会比单线程有更高的性能上限，但是在当今的云计算环境和挑战下，单机的上限已经不能满足我们了。
   ...
   网络带宽，作为第一个个瓶颈，已经渐渐消失了。因此，我们现在需要关心的重点是，如何更大限度的利用多核的优势以及网卡的速度。
   ...
   Redis Labs已经创造了在1ms延迟下，使用26个AWS上的EC2节点的[每秒50,000,000](https://redislabs.com/docs/linear-scaling-benchmark-50m-ops-sec/)的记录，也就是每实例每秒2,000,000次请求。
